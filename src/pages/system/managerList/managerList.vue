<template>
  <div>
    ...
    <Row>
      <Table
        size="large"
        no-data-text="无法检索到符合条件的其他普通管理员"
        stripe border
        :loading="loading"
        height="700"
    ...
    export default{
        ...
        methods:{
            init(index, name){
                ...
                this.getDepartment()
                axios({
                    url: apiRoot + "/system/managerList",
                    method: 'get'
                }).then((res) => {
            ...
            searchManager(){
                this.init("搜索成功！", this.department_search)
            },
            delPartner(index) {
            ...
            del(){
                ...
                axios({
                    url: apiRoot + '/system/managerDeletion?userId=' + this.data[this.deletingManager].userId,
                    method: 'get'
                }).then((res) => {
                    if(res.data.code == 200){
                        ...
                        this.init("刷新成功！", this.department_search)
                    }else{
                        this.$Message.error(res.data.message);
                        this.delete_loading = false;
                        this.modal_delete = false;
            ...
            createManager(){
                ...
                axios({
                    url: apiRoot + '/system/managerSetting',
                    method: 'post',
                    data:{
                        userList : this.items
                    }
